...
Dependency inputs live in `requirements.in`, `requirements-dev.in`, and
`deploy/requirements-pi.in`; their corresponding `.txt` files are exact,
transitive Python 3.10 locks generated by `pip-compile`. Use
`requirements-dev.txt` for a clone that will run tests and lint,
`requirements.txt` for research/training without developer tools, and the Pi
lock only for the deployed sensor. Upgrade intentionally by editing an `.in`
file and recompiling all three locks; CI rejects stale generated files.
